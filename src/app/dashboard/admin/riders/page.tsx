...
import RiderActionModal from "./components/RiderActionModal";
...
const RidersPage = () => {
  ...
  const meta = (ridersResponse as any)?.data?.meta || {
    total: 0,
    totalPage: 1,
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-md border border-gray-200 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage your delivery team in Barisal
          </p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat, i) => (
          <div
            key={i}
            className={`${stat.bg} ${stat.border} border rounded-md p-5 shadow-sm`}
          >
            <div className="flex items-center gap-4">
              <div
                className={`p-3 rounded-md bg-white shadow-sm ${stat.color}`}
              >
                <stat.icon size={22} />
              </div>
              <div>
            ...
            <div className="p-5 max-h-[70vh] overflow-y-auto">
              {selectedRiderAreas.length > 0 ? (
                selectedRiderAreas.map((area: any) => (
                  <div
                    key={area._id}
                    className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl mb-3"
                  >
                ...
                <p className="text-center py-10 text-gray-500">
                  No areas assigned to this rider
                </p>
              )}
            </div>
          </div>
