      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <select
            name="categoryID"
            required
            className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-md outline-none focus:border-[#4F46E5] text-sm"
            value={formData.categoryID}
            onChange={onChange}
          >
            <option value="">Select Category</option>
        ...
        {/* শুধুমাত্র সিঙ্গেল হলে ইউনিট দেখাবে */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            value={
              Array.isArray(formData.lowdown) ? formData.lowdown.join("\n") : ""
            }
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2 p-3 border border-gray-200 rounded-md bg-white min-h-[44px]">
          {(formData.tags || []).map((tag: string, idx: number) => (
            <span
              key={idx}
              className="flex items-center gap-1.5 px-3 py-1 bg-[#4F46E5]/10 text-[#4F46E5] text-xs font-semibold rounded-full"
            >
              {tag}
              <button
                type="button"
                onClick={() =>
                  onTagsChange(
                    formData.tags.filter((_: string, i: number) => i !== idx),
                  )
                }
        ...
        <p className="text-xs text-gray-400">
          Enter বা comma চাপলে tag যোগ হবে
        </p>
      </div>
    </div>
  );
