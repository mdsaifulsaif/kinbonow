...
export default function ReviewsPage() {
    ...
    const [toggleFeatured, { isLoading: isTogglingFeatured }] = useToggleFeaturedReviewMutation(); // New

    // Track which row is currently toggling, so only that row shows a spinner
    ...
    // New: toggle featured status, no body needed — just the id
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {isLoading ? (
                                [...Array(5)].map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={6} className="px-6 py-4"><div className="h-16 bg-gray-100 rounded w-full"></div></td>
                                    </tr>
                                ))
                            ) : reviews.length === 0 ? (
                                <tr>
                                ...
                                reviews.map((review: any) => (
                                    <tr
                                        key={review._id}
                                        className={`hover:bg-gray-50/50 align-top transition-colors ${review.isFeatured ? 'bg-amber-50/40' : ''}`}
                                    >
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-md bg-gray-100 flex-shrink-0 relative overflow-hidden">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <p className="text-xs font-bold text-gray-700">{review.user?.name || 'Anonymous'}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col items-center gap-1">
                                                <button
                                                    ...
                                                    className={`relative w-14 h-7 rounded-full transition-colors duration-300 flex items-center px-1 border
                                                        ${review.isFeatured
                                                            ? 'bg-amber-400 border-amber-400 justify-end'
                                                            : 'bg-gray-200 border-gray-200 justify-start'
                                                        }
                                                        ${togglingId === review._id ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}
                                                    `}
                                                >
                                                    <span className="w-5 h-5 rounded-full bg-white shadow-md flex items-center justify-center">
                                                        {togglingId === review._id ? (
                                                            <FiRefreshCw size={11} className="animate-spin text-gray-400" />
                                                        ) : review.isFeatured ? (
                                                            <FiStar size={11} className="text-amber-400 fill-amber-400" />
                                                        ) : (
                                                            <FiStar size={11} className="text-gray-300" />
                                                        )}
                                                    </span>
                                                </button>
                                                {review.isFeatured && (
